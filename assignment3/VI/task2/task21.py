...
def value_iteration(states, action_set, transition_table, gamma, epsilon):
    ...
    while start or delta >= epsilon:
        ...
        for i in range(len(states)):
            ...
            print(f"({state['direction']},{state['material']},{state['arrow']},{state['state']},{state['health']}):{convert_action_from_state(state,best_action)}=[{max_next_utility:.3f}]")
...
with open('utilities21.json', 'w', encoding='utf-8') as f:
