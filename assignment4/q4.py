...
# for state in states:
#     if state[0] == [1, 1] and (state[1] in [[0,1], [1, 0], [1, 1]]) and state[2] == 0:
#         start_states.append(state[3])
#         start_states_verbose.append(state)

start_states = []
probs = 0
start_probs = [0] * len(states)
start_states_verbose = []
for state in states:
    if state[0] == [0, 0]:
        if state[1] in [[0, 1], [0, 2], [1, 1], [1, 2]]:
            start_states.append([state, 0.4 * 0.25 * 0.5])
            start_probs[state[3]] =  0.4 * 0.25 * 0.5
            probs += 0.4 * 0.25 * 0.5
            # print('0.4:', state)
    if state[0] == [1, 3]:
        if state[1] in [[0, 1], [0, 2], [1, 1], [1, 2]]:
            start_states.append([state, 0.6 * 0.25 * 0.5])
            start_probs[state[3]] =  0.6 * 0.25 * 0.5
            probs += 0.6 * 0.25 * 0.5
            # print('0.6:', state)
...
print(f"start: {' '.join([str(x) for x in start_probs])}")
...
# for state in states:
#     if state[0] == [1, 1] and (state[1] in [[0,1], [1, 0], [1, 1]]) and state[2] == 0:
#         q2_start_states.append(state)

# print(q2_start_states)
...
# observation_probs = [0] * 6

# print('prob sum = ', sum(start_probs))
# print('max obs: ', np.argmax(observation_probs)+1)