...
selection_strategy = 'rowlette_wheel'
mutation_strategy = 'uniform_mutation'
...
def initialize_population_SUBMISSION():
    ...
    for i in range(pop_cnt):
        if not debug:
            population[i] = [x*np.random.uniform(0.97, 1.03) for x in overfit]
        if debug:
...
def compute_population_fitness(population):
    ...
    print(np.sort(verbose[-1]["train_error"]) , np.sort(verbose[-1]["val_error"]))
    print("\n")
